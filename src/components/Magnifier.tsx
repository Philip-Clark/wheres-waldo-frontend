...
interface State {
  ...
  // Rotation of the image relative to the window
  rotation: number;
}
...
export default class Magnifier extends PureComponent<Props, State> {
  ...
  onMouseMove = (e: MouseEvent): void => {
    ...
    if (this.imgBounds) {
      ...
      const x = (e.clientX - this.imgBounds.left) / target.clientWidth;
      const y = (e.clientY - this.imgBounds.top) / target.clientHeight;
      const [relX, relY] = rotatePoint(x, y, rotation);
      // const [relX, relY] = [x, y];
      const difX = relX - x;
      const difY = relY - y;
  ...
  onTouchMove = (e: TouchEvent): void => {
    ...
    if (this.imgBounds) {
      ...
      const x = (e.targetTouches[0].clientX - this.imgBounds.left) / target.clientWidth;
      const y = (e.targetTouches[0].clientY - this.imgBounds.top) / target.clientHeight;
      const [relX, relY] = rotatePoint(x, y, rotation);
        ...
        {this.imgBounds && (
          <div
            ...
            style={{
              ...
              backgroundSize: `${zoomFactor * this.imgBounds.width}% ${
